...
export default function handler(req, res) {
    ...
    try {
        const manifestPath = path.join(process.cwd(), 'scripts', 'manifest.json')
        const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'))
        const script = manifest[id]

        if (!script) {
            if (isBrowser) return res.status(404).send(`<h1 style="background:black;color:white;text-align:center;padding-top:20%">404 Not Found</h1>`)
